...
/datum/autowiki/surgery/generate()
	...
	for(var/datum/surgery_operation/operation as anything in unlocked_operations_alpha + locked_operations_alpha)
		...
		operation_data["description"] = escape_value(replacetext(operation.localizated_desc || operation.rnd_desc || operation.desc, "\"", "&quot;")) // PSYCHONAUT EDIT ADDITION - LOCALIZATION - Original: operation_data["description"] = escape_value(replacetext(operation.rnd_desc || operation.desc, "\"", "&quot;"))
