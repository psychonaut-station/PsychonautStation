...
#define is_playing(station) (station.track && world.time - station.track_started_at < station.track.duration)
...
/datum/radio_station/New(name, freq, list/musics, status_effect = /datum/status_effect/good_music)
	src.name = name
	src.freq = freq
	src.musics = shuffle(musics)
	src.id = "rs_[REF(src)]"
	src.status_effect = status_effect
...
/datum/radio_station/proc/play(mob/listener, cached_volume)
	var/datum/web_track/track = track
...
/datum/radio_station/proc/update_cached_volume(mob/listener)
	...
	if(LAZYLEN(sources))
		for(var/datum/weakref/source in sources)
			var/datum/component/radio_sound_player/player = source.resolve()
			if(!player)
				continue
...
/datum/radio_station/proc/get_next_music_index()
	. = 0
	if(music_index + 1 > length(musics))
		. = 1
	else
		. = music_index + 1
...
/datum/radio_station/proc/tune_in_radio(datum/component/radio_sound_player/player)
...
/datum/radio_station/proc/tune_out_radio(datum/component/radio_sound_player/player)
	var/list/listeners = SANITIZE_LIST(player.listeners)
	for(var/mob/listener in listeners)
		var/volume = listeners[listener]
