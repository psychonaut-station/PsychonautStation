...
/datum/controller/subsystem/dynamic/proc/send_intercept()
	...
	if(greenshift)
		priority_announce("Güvenlik ve istihbarat birimlerimizin bitmek bilmeyen çabaları sayesinde, şu anda [station_name()] mürettebatına yönelik herhangi bir tehdit bulunmamaktadır. Güvenli vardiyalar!", "Güvelik Raporu", SSstation.announcer.get_rand_report_sound(), color_override = "green")
	else
		if(SSsecurity_level.get_current_level_as_number() < SEC_LEVEL_BLUE)
			SSsecurity_level.set_level(SEC_LEVEL_BLUE, announce = FALSE)
		priority_announce("[SSsecurity_level.current_security_level.elevating_to_announcement]\n\nTüm iletişim konsollarına bir özeti gönderilmiştir.", "Güvenlik Seviyesi Yükseltildi.", ANNOUNCER_INTERCEPT, color_override = SSsecurity_level.current_security_level.announcement_color)
#endif
