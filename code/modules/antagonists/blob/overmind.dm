...
/mob/camera/blob/process()
	...
	if(announcement_time && (world.time >= announcement_time || blobs_legit.len >= announcement_size) && !has_announced)
		priority_announce("[station_name()] İstasyonunda 5. seviye biyolojik tehdit olduğu doğrulandı. Tüm personel salgını kontrol altına almalıdır.", "Biyolojik Tehlike Uyarısı", ANNOUNCER_OUTBREAK5)
		has_announced = TRUE
