...
/obj/item/radio
	...
	/// A very brief cooldown to prevent radio bark sounds from overlapping.
	COOLDOWN_DECLARE(bark_audio_cooldown)
...
/obj/item/radio/proc/talk_into_impl(atom/movable/talking_movable, message, channel, list/spans, datum/language/language, list/message_mods)
	...
	*/
	...
	if(isliving(talking_movable))
		...
		if(radio_noise && !HAS_TRAIT(talking_living, TRAIT_DEAF) && volume_modifier && signal.frequency != FREQ_COMMON && !LAZYACCESS(message_mods, MODE_SEQUENTIAL) && COOLDOWN_FINISHED(src, audio_cooldown) && !(GLOB.voices_enabled && talking_living.client?.prefs?.read_preference(/datum/preference/toggle/barks_enabled)))
