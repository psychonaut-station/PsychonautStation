...
ADMIN_VERB_ONLY_CONTEXT_MENU(cmd_admin_pm_context, R_ADMIN, "Admin PM Mob", mob/target)
	if(!ismob(target))
		to_chat(
			src,
...
ADMIN_VERB_ONLY_CONTEXT_MENU(cmd_mentor_pm_context, R_NONE, "Mentor PM Mob", mob/target)
	if(!ismob(target))
		to_chat(
			src,
			type = MESSAGE_TYPE_ADMINPM,
