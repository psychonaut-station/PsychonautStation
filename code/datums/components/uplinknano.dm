...
**/
/datum/component/nanouplink
	dupe_mode = COMPONENT_DUPE_UNIQUE
	/// Name of the uplink
	var/name = "nanotrasen uplink"
...
/datum/component/nanouplink/Initialize(
	...
	RegisterSignal(parent, COMSIG_ITEM_ATTACK_SELF, PROC_REF(nteract))
...
/datum/component/nanouplink/proc/handle_uplink_handler_update()
	SIGNAL_HANDLER
	SStgui.update_uis(src)

/// Adds telecrystals to the uplink. It is bad practice to use this outside of the component itself.
/datum/component/nanouplink/proc/add_telecrystals(telecrystals_added)
...
/datum/component/nanouplink/proc/set_telecrystals(new_telecrystal_amount)
	uplink_handler.telecrystals = new_telecrystal_amount

/datum/component/nanouplink/InheritComponent(datum/component/nanouplink/nanouplink)
	lockable |= nanouplink.lockable
	active |= nanouplink.active
	uplink_handler.uplink_flag |= nanouplink.uplink_handler.uplink_flag

/datum/component/nanouplink/Destroy()
	purchase_log = null
	return ..()

/datum/component/nanouplink/proc/load_tc(mob/user, obj/item/stack/telecrystal/telecrystals, silent = FALSE)
	if(!silent)
...
/datum/component/nanouplink/proc/OnAttackBy(datum/source, obj/item/item, mob/user)
	SIGNAL_HANDLER
	if(!active)
...
/datum/component/nanouplink/ui_state(mob/user)
	return GLOB.inventory_state

/datum/component/nanouplink/ui_interact(mob/user, datum/tgui/ui)
	active = TRUE
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "Nanouplink", name)
		// This UI is only ever opened by one person,
		// and never is updated outside of user input.                Ui burada
		ui.set_autoupdate(FALSE)
		ui.open()

/datum/component/nanouplink/ui_data(mob/user)
	if(!user.mind)
		return
	var/list/data = list()
...
/datum/component/nanouplink/ui_static_data(mob/user)
	var/list/data = list()
...
/datum/component/nanouplink/ui_assets(mob/user)
	return list(
		get_asset_datum(/datum/asset/json/uplink)
	)

/datum/component/nanouplink/ui_act(action, params, datum/tgui/ui, datum/ui_state/state)
	. = ..()
	if(.)
		return
	if(!active)
...
/datum/component/nanouplink/proc/implant_activation()
	SIGNAL_HANDLER
...
/datum/component/nanouplink/proc/implanting(datum/source, list/arguments)
	SIGNAL_HANDLER
...
/datum/component/nanouplink/proc/old_implant(datum/source, list/arguments, obj/item/implant/new_implant)
	SIGNAL_HANDLER
...
/datum/component/nanouplink/proc/new_implant(datum/source, datum/component/nanouplink/nanouplink)
	SIGNAL_HANDLER
...
/datum/component/nanouplink/proc/new_ringtone(datum/source, mob/living/user, new_ring_text)
	SIGNAL_HANDLER
...
/datum/component/nanouplink/proc/check_detonate()
	SIGNAL_HANDLER
...
/datum/component/nanouplink/proc/new_frequency(datum/source, list/arguments)
	SIGNAL_HANDLER
...
/datum/component/nanouplink/proc/pen_rotation(datum/source, degrees, mob/living/carbon/user)
	SIGNAL_HANDLER
...
/datum/component/nanouplink/proc/setup_unlock_code()
...
/datum/component/nanouplink/proc/generate_code()
	if(istype(parent,/obj/item/modular_computer/pda))
...
/datum/component/nanouplink/proc/failsafe(mob/living/carbon/user)
	if(!parent)
		return
