...
/datum/surgery/animal_revival
	name = "Animal Revival"
	desc = "A surgery used to revive animals."
	requires_bodypart_type = NONE
	surgery_flags = NONE
	target_mobtypes = list(/mob/living/basic/pet, /mob/living/simple_animal/pet)
	possible_locs = list(BODY_ZONE_HEAD)
	steps = list(
		/datum/surgery_step/incise,
...
/datum/surgery/animal_revival/can_start(mob/user, mob/living/target)
	if(!..())
		return FALSE
	if(target.stat != DEAD)
		return FALSE
...
/datum/surgery_step/animalrevive/tool_check(mob/user, obj/item/tool)
	. = TRUE
	if(istype(tool, /obj/item/shockpaddles))
		var/obj/item/shockpaddles/paddles = tool
		if((paddles.req_defib && !paddles.defib.powered) || !HAS_TRAIT(paddles, TRAIT_WIELDED) || paddles.cooldown || paddles.busy)
