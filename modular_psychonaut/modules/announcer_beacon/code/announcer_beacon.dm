...
/obj/item/announcer_beacon
	name = "Announcement Beacon"
	desc = "A beacon used for news and low-priority announcements."
	icon = 'modular_psychonaut/master_files/icons/obj/device.dmi'
...
/obj/item/announcer_beacon/emag_act(mob/user, obj/item/card/emag/emag_card)
	if(obj_flags & EMAGGED)
		return FALSE
	obj_flags |= EMAGGED
	return TRUE

/obj/item/announcer_beacon/attack_self(mob/user)
	if (!ishuman(user))
		return
...
/obj/item/announcer_beacon/examine(mob/user)
	. = ..()
