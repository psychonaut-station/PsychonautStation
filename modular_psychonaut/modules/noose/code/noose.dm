...
/obj/structure/noose
	...
	icon_state = "noose"
	icon = 'modular_psychonaut/modules/noose/icons/noose.dmi'
	layer = FLY_LAYER
	can_buckle = TRUE
	buckle_lying = 0
	anchored = TRUE
...
/obj/structure/noose/user_buckle_mob(mob/living/carbon/human/victim, mob/user, check_loc = TRUE)
	...
	if(do_after(user, user == victim ? 0 : 20 SECONDS, victim))
		if(buckle_mob(victim))
			...
			playsound(user.loc, 'modular_psychonaut/modules/noose/sound/noosed.ogg', 50, 1, -1)
...
/obj/structure/noose/process()
	...
	playsound(buckled_mob.loc, 'modular_psychonaut/modules/noose/sound/noose_idle.ogg', 25, 1, -3)
