/datum/job/animal
	...
	job_flags = JOB_CREW_MANIFEST | JOB_NEW_PLAYER_JOINABLE
	config_tag = "ANIMAL"
...
/datum/job/animal/get_spawn_mob(client/player_client, atom/spawn_point)
	...
	var/mob/living/prefered_spawn_type = GLOB.animal_job_types[animal_type] || spawn_type
	var/mob/living/spawn_instance = new prefered_spawn_type(player_client.mob.loc)
...
/datum/job/animal/proc/apply_prefs_job_animal(mob/living/spawned, client/player_client)
	...
	if(GLOB.current_anonymous_theme || CONFIG_GET(flag/force_random_names))
		spawned.fully_replace_character_name(null, pick(GLOB.animal_names))
		return
	spawned.apply_pref_name(/datum/preference/name/animal, player_client)

/datum/job/animal/proc/override_mob(mob/living/spawned)
	var/list/banned_components = list(/datum/component/tameable, /datum/component/obeys_commands)
	var/list/banned_elements = list(/datum/element/venomous)
	var/list/banned_traits = list(TRAIT_VENTCRAWLER_ALWAYS, TRAIT_SPACEWALK)
