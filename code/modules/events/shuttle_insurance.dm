...
/datum/round_event/shuttle_insurance/proc/answered()
	...
	if(insurance_message && insurance_message.answered == 1)
		...
		if(!station_balance?.adjust_money(-insurance_evaluation))
			priority_announce("Mekik sigortası için bize yeterince para göndermediniz. Bu, uzay uzmanlarının terimleriyle, dolandırıcılık olarak kabul edilir. Paranız bizde kalacak, dolandırıcılar!", sender_override = ship_name, color_override = "red")
			return
