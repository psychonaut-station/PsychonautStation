...
/datum/dna/proc/generate_unique_features()
	...
	if(features["ipc_chassis"])
		L[DNA_IPC_CHASSSIS_BLOCK] = construct_block(SSaccessories.ipc_chassis_list.Find(features["ipc_chassis"]), length(SSaccessories.ipc_chassis_list))
...
/datum/dna/proc/update_uf_block(blocknumber)
	...
	switch(blocknumber)
		...
		if(DNA_IPC_CHASSSIS_BLOCK)
			set_uni_feature_block(blocknumber, construct_block(SSaccessories.ipc_chassis_list.Find(features["ipc_chassis"]), length(SSaccessories.ipc_chassis_list)))
...
/mob/living/carbon/human/updateappearance(icon_update = TRUE, mutcolor_update = FALSE, mutations_overlay_update = FALSE)
	...
	if(dna.features["ipc_chassis"])
		dna.features["ipc_chassis"] = SSaccessories.ipc_chassis_list[deconstruct_block(get_uni_feature_block(features, DNA_IPC_CHASSSIS_BLOCK), length(SSaccessories.ipc_chassis_list))]
