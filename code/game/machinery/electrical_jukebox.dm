		...
		if(user)
			switch(track)
				...
				if(WEB_SOUND_ERR_INVALID_URL, WEB_SOUND_ERR_NOT_HTTPS)
					to_chat(user, span_warning("Given URL does not match any supported sites."))
				if(WEB_SOUND_ERR_JSON_RETRIEVAL)
					to_chat(user, span_warning("Given URL seems to be invalid. If you think this is a bug, please report this to the admins."))
