...
/obj/machinery/electrical_jukebox
	...
	icon = 'modular_psychonaut/modules/jukebox/icons/music.dmi'
...
/obj/machinery/electrical_jukebox/proc/add_queue(mob/user, input, request = FALSE)
	...
	if(request)
		...
		message_admins("[key_name(user, TRUE)] added web sound [track.webpage_url_html] to queue on [src] at [area_name]. [ADMIN_QUE(user)] [ADMIN_JMP(src)] [REMOVEQUEUE(src, track)] [BANJUKEBOX(src, user.client)]")
