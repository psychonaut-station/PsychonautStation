...
/obj/item/ipc_suit/examine(mob/user)
	. = ..()
	if(!brain)
		. += span_warning("There is no brain!")
	if(!tongue)
		. += span_warning("There is no tongue!")
	if(!eye)
		. += span_warning("There is no eye!")
	if(!ear)
		. += span_warning("There is no ear!")
	if(!monitor)
	...
	if(!head)
		. += span_warning("There is no head!")
	if(!chest)
		. += span_warning("There is no chest!")
	if(!leftarm)
		. += span_warning("There is no leftarm!")
	if(!rightarm)
		. += span_warning("There is no rightarm!")
	if(!leftleg)
		. += span_warning("There is no leftleg!")
	if(!rightleg)
		. += span_warning("There is no rightleg!")

// For testing
/obj/item/ipc_suit/full_except_brain
