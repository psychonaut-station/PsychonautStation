		...
		switch(pick(safety))//Chance based on the safety list.
			...
			if(2)
				switch(rand(1,3))//Food, drinks, or places. Only selectable once.
					if(1)
						. += LOCALE_LOWER_TEXT(pick(drinks)) // PSYCHONAUT EDIT ADDITION - LANGUAGE - Original: . += LOWER_TEXT(pick(drinks))
					if(2)
						. += LOCALE_LOWER_TEXT(pick(foods)) // PSYCHONAUT EDIT ADDITION - LANGUAGE - Original: . += LOWER_TEXT(pick(foods))
					if(3)
						. += LOCALE_LOWER_TEXT(pick(locations)) // PSYCHONAUT EDIT ADDITION - LANGUAGE - Original: . += LOWER_TEXT(pick(locations))
				safety -= 2
			if(3)
				switch(rand(1,4))//Abstract nouns, objects, adjectives, threats. Can be selected more than once.
					if(1)
						. += LOCALE_LOWER_TEXT(pick(nouns)) // PSYCHONAUT EDIT ADDITION - LANGUAGE - Original: . += LOWER_TEXT(pick(nouns))
					if(2)
						. += LOCALE_LOWER_TEXT(pick(objects)) // PSYCHONAUT EDIT ADDITION - LANGUAGE - Original: . += LOWER_TEXT(pick(objects))
					if(3)
						. += LOCALE_LOWER_TEXT(pick(adjectives)) // PSYCHONAUT EDIT ADDITION - LANGUAGE - Original: . += LOWER_TEXT(pick(adjectives))
					if(4)
						. += LOCALE_LOWER_TEXT(pick(threats)) // PSYCHONAUT EDIT ADDITION - LANGUAGE - Original: . += LOWER_TEXT(pick(threats))
		if(!return_list)
			if(words == 1)
				. += "."
			else
