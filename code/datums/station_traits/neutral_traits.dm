...
/datum/station_trait/eddl/proc/on_job_after_spawn(datum/source, datum/job/job, mob/living/spawned, client/player_client)
	...
	if((job.departments_bitflags & DEPARTMENT_BITFLAG_SECURITY))
		...
		spawned.grant_language(language = /datum/language/department/sec, language_flags = ALL, source = LANGUAGE_MIND)
		spawned.remove_blocked_language(/datum/language/department/sec)
	else if((job.departments_bitflags & DEPARTMENT_BITFLAG_SERVICE))
		spawned.remove_all_languages()
		spawned.grant_language(language = /datum/language/department/ser, language_flags = ALL, source = LANGUAGE_MIND)
		spawned.remove_blocked_language(/datum/language/department/ser)
	else if((job.departments_bitflags & DEPARTMENT_BITFLAG_CARGO))
		spawned.remove_all_languages()
		spawned.grant_language(language = /datum/language/department/car, language_flags = ALL, source = LANGUAGE_MIND)
		...
		spawned.remove_all_languages()
		spawned.grant_language(language = /datum/language/department/eng, language_flags = ALL, source = LANGUAGE_MIND)
		spawned.remove_blocked_language(/datum/language/department/eng)
	else if((job.departments_bitflags & DEPARTMENT_BITFLAG_SCIENCE))
		spawned.remove_all_languages()
		spawned.grant_language(language = /datum/language/department/sci, language_flags = ALL, source = LANGUAGE_MIND)
		spawned.remove_blocked_language(/datum/language/department/sci)
	else if((job.departments_bitflags & DEPARTMENT_BITFLAG_MEDICAL))
		spawned.remove_all_languages()
		spawned.grant_language(language = /datum/language/department/med, language_flags = ALL, source = LANGUAGE_MIND)
		spawned.remove_blocked_language(/datum/language/department/med)
	if((job.departments_bitflags & DEPARTMENT_BITFLAG_COMMAND))
		spawned.grant_language(language = /datum/language/common, language_flags = ALL, source = LANGUAGE_MIND)
