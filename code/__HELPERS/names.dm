		...
		switch(pick(safety))//Chance based on the safety list.
			...
			if(2)
				switch(rand(1,3))//Food, drinks, or places. Only selectable once.
					if(1)
						. += locale_lowertext(pick(drinks))
					if(2)
						. += locale_lowertext(pick(foods))
					if(3)
						. += locale_lowertext(pick(locations))
				safety -= 2
			if(3)
				switch(rand(1,4))//Abstract nouns, objects, adjectives, threats. Can be selected more than once.
					if(1)
						. += locale_lowertext(pick(nouns))
					if(2)
						. += locale_lowertext(pick(objects))
					if(3)
						. += locale_lowertext(pick(adjectives))
					if(4)
						. += locale_lowertext(pick(threats))
		if(!return_list)
			if(words == 1)
				. += "."
			else
